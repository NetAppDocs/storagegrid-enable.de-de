...
. Öffnen Sie den StorageGRID Grid Manager.
. Wählen Sie in der oberen rechten Ecke das Fragezeichen und dann API-Dokumentation aus.
. Klicken Sie oben rechts auf den Link zur Seite „Private API-Dokumentation“.
. Erweitern Sie ilm-advanced.
. Wählen Sie GET ilm-advanced.
. Wählen Sie „Ausprobieren“ und dann „Ausführen“.
. Überprüfen Sie das Antwortergebnis.
+
.. Wenn die Werte null sind, bedeutet dies, dass die standardmäßigen ILM-Advanced-Werte verwendet werden.
.. Wenn die Werte nicht null sind, bedeutet dies, dass benutzerdefinierte erweiterte ILM-Werte verwendet werden.  Kopieren Sie die gesamte Ausgabe nach „data“:, beginnend mit { bis zum vorletzten }.
+
... Speichern Sie es in einem Texteditor.
+
Beispielantwort:

+
image:veeam-bp/get_ilm_adv_before.png["GET ILM-Advanced-Ausgabe"]





. Wählen Sie PUT ilm-advanced.
. Wählen Sie „Ausprobieren“ aus, um mit der Bearbeitung des API-Texts zu beginnen.
+
.. Standardmäßig enthält der API-Text Standardwerte und keine zuvor konfigurierten benutzerdefinierten Werte.  Aus diesem Grund ist es SEHR wichtig, die Schritte 5 bis 7 auszuführen.


. Wenn in den Schritten 5–7 nicht standardmäßige Werte gefunden werden, ersetzen Sie den API-Text durch die in Schritt 7 gespeicherte Ausgabe. .  Andernfalls, wenn die Werte in den Schritten 5–7 null waren, lassen Sie den API-Text unverändert.
. Passen Sie die folgenden Parameter im API-Body-Feld an:
+
.. Setzen Sie den synchronen Wert auf „false“.
+
Beispiel für API-Textkörper:

+
image:veeam-bp/put_ilm_adv.png["PUT ILM-Advanced-Input"]



. Wenn Sie fertig sind, wählen Sie Ausführen
...
* link:https://docs.netapp.com/us-en/storagegrid/["NetApp StorageGRID -Produktdokumentation"^]
