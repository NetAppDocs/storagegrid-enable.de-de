...
----
+
Sie können nun die Autorisierungs-Token-Zeichenfolge verwenden, um über cURL auf Metriken zuzugreifen. Der Zugriff auf Metriken erfolgt ähnlich wie in Abschnitt link:monitoradvanced-monitor-storagegrid.html#export-metrics-through-the-api["Erweiterte Überwachung in StorageGRID"]. Zur Veranschaulichung wird jedoch ein Beispiel mit GET /grid/metric-labels/{label}/values in der Kategorie Metriken gezeigt.
