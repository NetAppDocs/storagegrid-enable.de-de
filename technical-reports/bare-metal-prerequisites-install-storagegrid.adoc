...
NOTE: Die Zahlen in der Tabelle beziehen sich auf jeden StorageGRID-Diensttyp, nicht auf das gesamte Grid oder jeden einzelnen physischen Host. Abhängig von den Bereitstellungsoptionen werden Zahlen für jeden physischen Host in link:bare-metal-prerequisites-install-storagegrid.html#physical-host-layout-and-requirements["Physisches Host-Layout und Anforderungen"] später in diesem Dokument berechnet. ++ ++ Die mit einem Sternchen gekennzeichneten Pfade oder Dateisysteme werden vom Installationsprogramm im StorageGRID-Container selbst erstellt. Eine manuelle Konfiguration oder Dateisystemerstellung durch den Administrator ist nicht erforderlich, jedoch benötigen die Hosts Blockgeräte, um diese Anforderungen zu erfüllen. Anders ausgedrückt sollte das Blockgerät mithilfe des Befehls  `lsblk` angezeigt werden, aber nicht im Host-Betriebssystem formatiert oder eingebunden sein. ++ ++
...
|===
In diesem Beispiel werden die in der folgenden Tabelle aufgeführten Festplattengrößen pro Containertyp benötigt. Die Anforderungen pro physischem Host sind in link:bare-metal-prerequisites-install-storagegrid.html#physical-host-layout-and-requirements["Physisches Host-Layout und Anforderungen"] später in diesem Dokument beschrieben.
...
Wenn Sie keinen iSCSI-Speicher verwenden, muss sichergestellt sein, dass Host1, Host2 und Host3 Blockgeräte mit ausreichender Größe enthalten, um ihre Anforderungen zu erfüllen. Siehe link:bare-metal-prerequisites-install-storagegrid.html#disk-sizes-per-container-type["Festplattengrößen pro Containertyp"] für die Speicheranforderungen von Host1, Host2 und Host3.
...
----
. Mithilfe des Initiatornamens aus Schritt 2 werden die LUNs auf Ihrem Speichermedium (in der Anzahl und Größe, wie in der link:bare-metal-prerequisites-install-storagegrid.html#storage-requirements["Storage-Anforderungen erfüllt"] Tabelle angegeben) jedem Speicherknoten zugeordnet.
